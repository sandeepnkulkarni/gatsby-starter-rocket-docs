...
In this tutorial, we will learn about how to add worker node to an existing Kubernetes Cluster. To learn more about how to create a single node Kubernetes cluster, please refer to section [Create single node Kubernetes cluster on Ubuntu using kubeadm on Google Cloud Platform (GCP)](/kubernetes/single-node-k8s-ubuntu-gcp-kubeadm). Same steps which are mentioned below could be also performed on any Kubernetes cluster created on Ubuntu whether they are hosted on Google Cloud Platform (GCP) or not.
